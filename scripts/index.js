...
const validateProfileEdit = new FormValidator(validateObj, util.submitFormProfileEditPopup); // создаем дубликат FormValidator редактирования профиля
const validateCardAdd = new FormValidator(validateObj, util.submitFormAddNewPlacePopup); // создаем дубликат FormValidator добавления новой карточки
...
const profilePopup = new PopupWithForm('.popup_profile-edit', {
    handleSubmit: () => {
        userInfo.setUserInfo(util.inputNameProfileEdit.value, util.inputAboutProfileEdit.value); // значения из интпутов сохраняем при помощи метода setUserInfo класса UserInfo
        profilePopup.close();
    }
});
...
const cardAddPopup = new PopupWithForm('.popup_img-add', {
    handleSubmit: () => {
        const cardName = util.inputPlaceNameAddNewPlace.value;
        const cardLink = util.inputLinkAddNewPlace.value;
...
util.buttonProfileEdit.addEventListener('click', () => { // слушатель кнопки открытия POPUP редактирования профиля
    profilePopup.open();
    const { name, about } = userInfo.getUserInfo(); // используем класс UserInfo что бы передавать текучие значения тайтлов в инпуты
    util.inputNameProfileEdit.value = name;
    util.inputAboutProfileEdit.value = about;
    validateProfileEdit.deactivateValidation(); // чистим ошибки валидации
});

util.buttonAddNewPlace.addEventListener('click', () => { // слушатель кнопки открытия POPUP добавления нового места 
    cardAddPopup.open();
    validateCardAdd.deactivateValidation();
});